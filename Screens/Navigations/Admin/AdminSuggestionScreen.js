...
import { remoteDBSuggestionFeedback } from '../../../Database/pouchDb';
import { Calendar } from 'react-native-calendars';

  export const AdminSuggestionScreen = () => {
    ...
    const handleDateSelect = async (date) => {
      ...
      try {
        const result = await remoteDBSuggestionFeedback.allDocs({
          include_docs: true,
          attachments: true
        });
        
        const modifiedArr = result.rows?.map(item => item.doc) || [];
        const filteredData = modifiedArr.filter(item => item.FeedbackorSuggestion === 'Suggestion');
...
const Suggestion = async() => {
  ...
  return (
    <ImageBackground style={styles.container} source={require('../../../Assets/Img/Background_image.png')}>
      ...
      refreshControl={
        ...
        <View style={[styles.status, { backgroundColor: 'lime' }]}>
          <Text style={styles.text}>Suggestion</Text>
        </View>
        <FlatList
    showsHorizontalScrollIndicator={false}
    data={suggestion.filter((item) => item.Department.includes(searchQuery))}
  ...
  />
      ...
      <Modal visible={fullinfomodal} animationType="slide">
        <View>
          ...
          <Text style={styles.modalText}>Concern:  {fullinfomodaldata.FeedbackorSuggestion}</Text>
  ...
  export default AdminSuggestionScreen;